...
```
```{r}
st <- read.csv("/Users/meltemozcan/Desktop/PSYC 626/TwitterAbortion/external/states.csv")
states <- st[,1]
states_abbr <- st[,2]
...
race <- c("White_alone", "Black_alone", "Native_alone", "HIPI_alone" , 
              "Asian_alone", "other_race_alone", "two_or_more_races")
race_dat[, race] <- race_dat[, race] / race_dat$total_race
...
eduDatCompiled <- eduDatComp %>% 
  ...
  dplyr::select(-c(edu_total, edu_ov25_total))
...
censusdatw <- censusdatw %>% dplyr::select(state, everything())
censusdatw <- censusdatw %>% dplyr::select(-c(NAME, STATE))
dim(censusdatw)
```


```{r}
write.csv(censusdatw, "/Users/meltemozcan/Desktop/PSYC 626/TwitterAbortion/external/ACS_2021_censusData.csv")
#dim(censusdat)
```
